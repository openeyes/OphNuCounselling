...
?>
 ...
 true, 'empty'=>'- Please select -'))?>
		...
		<div id="div_Element_OphNuCounselling_Counselling_relationship_2_name" class="row field-row"<?php if ($element->caregivers_present_id != 1) {?> style="display: none"<?php }?>>
			...
			<div class="large-9 column end">
				<div class="row field-row">
					...
					<div class="large-3 column end">
						<?php echo $form->dropDownList($element, 'relationship_2_id', CHtml::listData(OphNuCounselling_CareGivers_Relationship::model()->findAll(array('order'=> 'display_order asc')),'id','name'),array('nowrapper' => true, 'empty'=>'- Please select -'))?>
					</div>
				</div>
			</div>
		</div>
		...
		<?php echo $form->multiSelectList($element, 'pre_emotions', 'pre_emotions', 'id', $this->getEmotionList($element,'pre_emotions'), array(), array('empty' => '- Please select -', 'label' => 'Pre-counseling emotional status'),false,false,null,false,false,array('label' => 3, 'field'=>4))?>
		...
		<?php echo $form->multiSelectList($element, 'post_emotions', 'post_emotions', 'id', $this->getEmotionList($element,'post_emotions'), array(), array('empty' => '- Please select -', 'label' => 'Post-counseling emotional status'),false,false,null,false,false,array('label' => 3, 'field' => 4))?>
	</div>
